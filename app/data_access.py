"""SQLite read helpers for the Streamlit UI."""
...
def load_locations(
    ...
    try:
        if parent_slug is None:
            rows = conn.execute(
                "SELECT slug, name FROM locations WHERE type=? ORDER BY name",
                (loc_type,),
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT slug, name FROM locations WHERE type=? AND parent_slug=? ORDER BY name",
...
def load_crawl_progress(db_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        statuses = {row["status"]: row["jobs"] for row in status_rows}
        ...
        rows_written = conn.execute(
            "SELECT COALESCE(SUM(rows_written), 0) FROM scrape_jobs"
        ).fetchone()[0]
...
def load_matching_jobs(
    ...
    try:
        ...
        for target_type, target_slugs in target_slugs_by_type.items():
            placeholders = ",".join("?" for _ in target_slugs)
            target_clauses.append(
                f"(target_type=? AND target_slug IN ({placeholders}))"
            )
            params.extend([target_type, *target_slugs])
        if target_clauses:
            query += f" AND ({' OR '.join(target_clauses)})"
        if city_slugs:
            placeholders = ",".join("?" for _ in city_slugs)
...
def _facet_text(conn: Any, source_url: str) -> str:
    facets = conn.execute(
        """SELECT facet_type, name, slug
...
def load_businesses(
    ...
    try:
        ...
        query += " ORDER BY b.scraped_at DESC, b.business_name LIMIT ?"
        params.append(limit)
        rows = conn.execute(query, params).fetchall()
        businesses: list[dict[str, Any]] = []
        for row in rows:
            item = dict(row)
            item["matched_facets"] = _facet_text(conn, item["source_url"])
